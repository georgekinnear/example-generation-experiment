...
question_details = read_csv("../data-raw/study2/FAC_LGE2-tasks.csv") %>%
...
# Experimental groups
participant_groups = read_csv("../data-raw/study2/FAC_LGE2-groups.csv") %>% 
  clean_names() %>% 
  separate(email_address, c("username", NA), sep = "@") %>% 
  select(username, group)

# Consent
participants = read_csv("../data-raw/study2/FAC_LGE2-Course design research-grades.csv") %>% 
  clean_names() %>% 
...
all_attempt_data =
  list.files(path = "../data-raw/study2/",
...
write_attempts_csv = function(DF) {
  write.csv(DF,
            paste0("data-clean/study2/processed-attempts-",unique(DF$question_name),".csv"),
            row.names = FALSE)
  return(DF)
}
attempt_data %>% 
...
attempt_data %>%
  write_rds("data-clean/FAC_LGE2-attempt_data.rds")
