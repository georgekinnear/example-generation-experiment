...
groupcolours = c("G-CG" = heathers[1],
                 "G-GC" = "#6f004a", # darker heathers1
                 "C-CG" = heathers[2],
                 "C-GC" = "#a34e03" # darker heathers2
                 )
...
detailed_scores_summary <- detailed_scores_summary %>% 
  ...
  left_join(
    # add the scores from the initial learning tasks
    detailed_scores %>% 
      filter(attempt_number == 1) %>%
      filter(str_detect(question_name, "^[CG]")) %>% 
      group_by(username, group, overall_group) %>% 
      summarise(across(c("marks_attained"), sum, na.rm = TRUE), .groups = "drop") %>% 
      rename_with(~ str_replace(., "marks_", "learning_task_marks_")),
    by = c("username", "group", "overall_group")
  )
```
...
fit_model <- function(data) {
  stan_glm(
    ...
    data = data,
    seed = 123,
    # prior = normal(6, 2.5),  uncomment this line to use an informed prior
...
Here are the details of the weakly informative prior produced by the defaults in `rstanarm`:

```{r}
bayestestR::describe_prior(model) %>% basic_kable()

bayestestR::simulate_prior(model) %>% pivot_longer(everything(), names_to = "group", values_to = "marks_attained") %>% ggplot(aes(y = group, x = marks_attained)) + ggdist::stat_halfeye(alpha = 0.5)
```
These are very broad priors (indeed, going beyond the range of possible scores). However, replacing them with more informed priors (e.g., `prior = normal(6, 2.5)`) or even flat priors (`prior = NULL`) does not substantially change the results: the various HDIs have slightly different endpoints, but the conclusions remain the same.
...
ggsave("figs/FIG_study1_marks_attained.eps", device = cairo_ps, width = 12, height = 7, units = "cm")
```
...
p2 <- tab1 %>% 
  ...
  labs(y = "", x = NULL) +
  scale_y_discrete(labels = c("n" = "N", "mean" = "Estimated mean", "estimate_hdi" = "\n95% HDI\nfor mean")) +
  theme_minimal() +
  theme(axis.line = element_blank(), axis.ticks = element_blank(), axis.text.x = element_blank(),
        panel.grid = element_blank(), strip.text = element_blank())
  
p1 / p2 +  plot_layout(heights = c(8, 2.5))
ggsave("figs/FIG_study1_marks_attained_with_table.eps", device = cairo_ps, width = 15, height = 10, units = "cm")
```
...
p_diffs
ggsave("figs/FIG_study1_marks_attained_effect.eps", device = cairo_ps, width = 14, height = 5, units = "cm")
```
